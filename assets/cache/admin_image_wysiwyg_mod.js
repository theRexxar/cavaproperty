...
function insertImage(file, alt)
{
	...
	window.parent.instance.insertHtml('<img class="admin-image" style="float: '+get_float()+';" src="{ site_url }files/thumb/' + file + '/' + img_width + '" alt="' + alt + '" />');
    windowClose();
}
...
function insertFile(id, title)
{
	...
	window.parent.instance.insertHtml('<a class="admin-file" href="{ site_url }files/download/' + id + '">' + title + '</a>');
    windowClose();
}
...
(function($)
{
	$(function()
	{
		function detectFile()
		{
			...
			if( ! element.hasClass('admin-image')) return false;

			$('#current_document').load(SITE_URL + SITE_AREA + '/content/files/wysiwyg/file/ajax_get_file', {
