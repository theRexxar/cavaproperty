...
class Contact_mail extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('contact_mail_model', null, true);
		$this->lang->load('contact_mail');
		
		Template::set_block('sub_nav', 'contact_mail/_sub_nav');
	}
	...
	public function index()
	{
		...
		if (isset($_POST['delete']))
		{
			...
			if (is_array($checked) && count($checked))
			{
				...
				{
					$result = $this->contact_mail_model->delete($pid);
				}
				...
				{
					Template::set_message(lang('contact_delete_failure') . $this->contact_mail_model->error, 'error');
				}
			}
		}
		...
		$records = $this->contact_mail_model->limit($this->limit, $offset)->find_all();

                
        // Pagination
		$this->load->library('pagination');

		$total_article = $this->contact_mail_model->count_all();

		$this->pager['base_url'] 		= site_url(SITE_AREA .'/content/contact/contact_mail/index');
		...
		Template::set('toolbar_title', 'Manage Contact Mail Form');
		Template::render();
	}
	...
	*/
	public function detail()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/content/contact/contact_mail');
		}
        
        $form_detail = $this->contact_mail_model->find_by("id", $id);
                
        Assets::add_module_css('contact', 'contact.css');                                                

		Template::set('form_detail', $form_detail);
		Template::set('toolbar_title', 'Detail Mail Form');
		Template::render();
	}
	...
	public function create()
	{
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_contact())
			{
				...
				Template::redirect(SITE_AREA .'/content/contact/contact_mail');
			}
			else
			{
				Template::set_message(lang('contact_create_failure') . $this->contact_mail_model->error, 'error');
			}
		}
        ...
        Assets::add_js($this->load->view('contact_mail/form_js', null, true), 'inline'); 

		Template::set('toolbar_title', lang('contact_create') . ' Contact Mail Form');
		Template::render();
	}
	...
	*/
	public function edit()
	{
		...
		if (isset($_POST['save']))
		{
			...
			{
				Template::set_message(lang('contact_edit_failure') . $this->contact_mail_model->error, 'error');
			}
		}
		else if (isset($_POST['delete']))
		{
			...
			if ($this->contact_mail_model->delete($id))
			{
				// Log the activity
		...
		Template::set('contact', $this->contact_mail_model->find($id));
		...
		Template::set('toolbar_title', lang('contact_edit') . ' Contact Mail Form');
		Template::render();
	}
	...
	*/
	private function save_contact($type='insert', $id=0)
	{
		...
		if ($type == 'insert')
		{
			$id = $this->contact_mail_model->insert($data);
		...
		{
			$return = $this->contact_mail_model->update($id, $data);
		}
