...
class content extends Admin_Controller {
	...
	public function index()
	{
		...
		if (isset($_POST['delete']))
		{
			...
			if (is_array($checked) && count($checked))
			{
				...
				{
					$result = $this->project_model->delete($pid);
				}
				...
				{
					Template::set_message(lang('project_delete_failure') . $this->project_model->error, 'error');
				}
			}
		}

		$records = $this->project_model->order_by('title', 'asc')->find_all();

		Template::set('records', $records);
		Template::set('toolbar_title', 'Manage Project Category');
		Template::render();
	}
	...
	public function create()
	{
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_project())
			{
				...
				Template::redirect(SITE_AREA .'/content/project');
			}
			else
			{
				Template::set_message(lang('project_create_failure') . $this->project_model->error, 'error');
			}
		}
		...
		Template::set('toolbar_title', lang('project_create') . ' Project Category');
		Template::render();
	}
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/content/project');
		}
		...
		if (isset($_POST['save']))
		{
			...
			{
				Template::set_message(lang('project_edit_failure') . $this->project_model->error, 'error');
			}
		}
		...
		Template::set('project', $this->project_model->find($id));
		...
		Template::set('toolbar_title', lang('project_edit') . ' Project Category');
		Template::render();
	}
	...
	*/
	...
	public function _check_title($title, $id = null)
	{
		...
		return $this->project_model->check_exists('title', $title, $id);
	}
	...
	public function _check_slug($slug, $id = null)
	{
		...
		return $this->project_model->check_exists('slug', $slug, $id);
	}
	...
	*/
	private function save_project($type='insert', $id=0)
	{
		...
		{
			$return = $this->project_model->update($id, $data);
		}
