<div id="upload-box">
	...
	<?php echo form_open_multipart(SITE_AREA.'/content/files/fileman/upload'); ?>
		<?php echo form_hidden('redirect_to', 'file'); ?>
		<p>
			<input name="name" value="<?php echo set_value('name', lang('files.name_label')); ?>" type="text">
		...
		<p>
			<?php echo form_submit('button_action', lang('bf_action_save'), 'class="button"'); ?>
		</p>
	<?php echo form_close(); ?>
</div>
    ...
    <div id="files_left_pane">
		...
		<?php if ( ! $folder->parent_id): ?>
			<li id="folder-id-<?php echo $folder->id; ?>" class="<?php echo $current_folder && $current_folder->id == $folder->id ? 'current' : ''; ?>">
				<?php echo anchor(SITE_AREA."/content/files/fileman/file/{$folder->id}", $folder->name, 'title="'.$folder->slug.'"'); ?>
			</li>
		<?php endif; ?>
		<?php endforeach; ?>
		<?php if ($folders): ?>
			<li class="upload">
				<?php echo anchor(SITE_AREA."/content/files/fileman/file/#upload", lang('files.upload_title'), 'title="upload"'); ?>
			</li>
		<?php endif; ?>
		</ul>
	</div>
			...
			<table class="table-list" border="0">
				<thead>
					<tr>
						...
						<th><?php echo lang('fileman.meta.mime'); ?></th>
					</tr>
				</thead>
				<tbody>
