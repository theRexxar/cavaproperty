<div id="upload-box">
	...
	<?php echo form_open_multipart(SITE_AREA.'/content/files/wysiwyg/upload',array('class'=>'form-horizontal')); ?>
	...
	<div id="files_left_pane">
		...
		<?php if ( ! $folder->parent_id): ?>
			<li id="folder-id-<?php echo $folder->id; ?>" class="<?php echo $current_folder && $current_folder->id == $folder->id ? 'current' : ''; ?>">
				<?php echo anchor(SITE_AREA."/content/files/wysiwyg/image/{$folder->id}", $folder->name, 'title="'.$folder->slug.'"'); ?>
			</li>
		<?php endif; ?>
		<?php endforeach; ?>
		<?php if ($folders): ?>
			<li class="upload">
				<?php echo anchor(SITE_AREA."/content/files/wysiwyg/image/#upload", lang('files.upload_title'), 'title="upload"'); ?>
			</li>
		<?php endif; ?>
		</ul>
	</div>
