...
class Fileman extends Admin_Controller {
	...
	public function upload()
	{
        ...
        Template::set_message(sprintf(lang('files.create_success'), $img['upload_data']['file_name']), 'success');
			...
			redirect(SITE_AREA."/content/files/fileman/{$this->input->post('redirect_to')}/{$this->input->post('folder_id')}?target={$this->input->get('target')}");
		}
	}
	...
	public function image($id = 0)
	{
		...
		Assets::add_module_js('files', 'fileman.js');
		Assets::add_module_css('files', 'fileman.css');

		Template::set('data', $data);
		Template::set('toolbar_title', 'Images');
		Template::set_view('fileman/image');
		Template::render('fileman', true);
	}
	...
	public function file($id = 0)
	{
		...
		Template::set_view('fileman/file');
		Template::render('fileman', true);
	}
	...
	public function ajax_get_file()
	{
		...
		$this->load->view('fileman/ajax_current_file', array(
			'file'		=> $file,
			'folders'	=> $folders
		));
	}
